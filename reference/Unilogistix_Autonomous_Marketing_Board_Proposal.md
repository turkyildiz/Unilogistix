...
Prepared for: Founder and Board of Unilogistix  
...
> **Decision status:** Everything newly proposed in this document is submitted for board discussion. Receiving, discussing, or storing this file does not approve a budget, select a vendor, authorize customer contact, grant production access, or activate a campaign. Existing authenticated founder directions remain distinct from these proposed implementation details. No repository or live service was changed to produce this document.
...
## 13. Illustrative first-product scenario

The following is hypothetical and does not authorize Safe Goes or assert any released capability.

Assume an approved tracking product has a verified dispatcher workflow. UNI tests a general feature message against a demonstration of that workflow. Both campaigns lead to tested landing pages and an operational onboarding process.

| Illustrative campaign | Advertising spend | Leads | New paying customers | Advertising cost per new paying customer |
| --- | ---: | ---: | ---: | ---: |
| General feature message | $300 | 30 | 1 | $300 |
| Verified workflow demonstration | $300 | 12 | 3 | $100 |

The second campaign is a promising signal, not a statistically established winner. The four total customers provide limited evidence. These costs exclude other acquisition expenses. UNI evaluates activation, retention, refunds, support burden, and the predeclared evidence rule before expanding.

Suppose customers then report duplicate notifications. Support verifies identity and links the reports to permitted telemetry. Product reproduces the problem, engineering adds a regression test and fix, independent review checks it, and a controlled release measures both duplication and alert latency. Marketing updates affected explanations only after verification and sends appropriate follow-ups.

Success is the connected outcome: suitable customer acquired, product used, payment reconciled, issue resolved, and measured improvement delivered without routine founder intervention.
...
**Gate A — Documented and authorized scope.** Board records the initial product, market, limits, review rules, and implementation authority. Documentation approval alone does not activate live outreach.
...
## 17. Proposed repository documentation placement

Suggested primary location after board disposition:

`books/BOOK-06-BUSINESS-OPERATIONS/AUTONOMOUS-MARKETING.md`

Retain a dated discussion copy or decision reference. Do not silently replace the existing constitution, autonomy policy, or customer policy with this proposal.
